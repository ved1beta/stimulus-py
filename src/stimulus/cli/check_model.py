...
def main(
    ...
    learner = StimulusTuneWrapper(
        ...
        _debug=_debug_mode,
    )  # TODO this version of pytorch does not support relative paths, in future maybe good to remove abspath
    ...
    for i in range(len(grid_results)):
        ...
        if not result.error:
            print(f"Trial finishes successfully with metrics{result.metrics}.")
        else:
