...
def get_args():
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="FILE",
        help="The json config file that hold all transform - split - parameter info",
    )
    parser.add_argument(
        "-d",
        "--out_dir",
        ...
        default="./",
        metavar="DIR",
        help="The output dir where all json files are written to. Output Json will be called input_json_nam-#[number].json. Default -> ./",
    )
...
def interpret_json(input_json: dict) -> list:
    ...
    function_call_dict = {
        "column_wise": schema.transform_column_wise_combination,
        "all_combinations": schema.transform_all_combination,
    }
...
def main(config_json: str, out_dir_path: str) -> str:
    ...
    for i, interpreted_json in enumerate(list_json):
        ...
        with open(experiment_file_path, "w") as experiment_file, open(transform_file_path, "w") as transform_file:
            ...
            json.dump(
                {"experiment": interpreted_json["experiment"], "transform": interpreted_json["transform"]},
                transform_file,
            )
